...
namespace Tridimensional.Puzzle.Service.ServiceImplementation.SliceStrategy
{
    public class DefaultSliceStrategy : AbstractSliceStrategy
	{
        ...
        public override Vector2[,] GetVertexes(FormationContract formationContract)
        {
            var vertexes = new Vector2[formationContract.Rows + 1, formationContract.Columns + 1];

            for (var i = 0; i < vertexes.GetLength(0); i++)
            {
                for (var j = 0; j < vertexes.GetLength(1); j++)
                {
                    vertexes[i, j] = new Vector2(1.0f * formationContract.Width * j / formationContract.Columns - formationContract.Width / 2, 1.0f * formationContract.Height * i / formationContract.Rows - formationContract.Height / 2);
                }
            }
